...
<template>
  <div class="animated fadeIn">
      <codemirror v-model="value2" :options="editorOption"></codemirror>
    <a>下载</a>
  </div>
</template>
...
<script>
  ...
  exportRaw(name, data) {
    ...
    var link = window.document.getElementsByTagName('a')[0];
    link.download = "file";
    link.href = URL.createObjectURL(blob);
  }
    },
    created(){
      this.exportRaw('template.txt',this.value2)
      let str = 'http://stable.admin.migu.cn/ms/login?callbackURL=http%3A%2F%2Fstable.admin.migu.cn%2Fms-admin%2Fstatic%2Fupgrade%2Fpublish.html';
      // let parsedUrl = new URL('https://cn.bing.com?id=123v');
      let parsedUrl = new URL(str);
      console.log(parsedUrl.searchParams.get('callbackURL'))
      // parsedUrl.searchParams.set("name",'huanglirong')
      parsedUrl.toString()
      console.log(window.URL);

      window.onload = this.createDownloadFile;
    },
    mounted () {
      setTimeout(() => {
        this.code += '\n\n\t~haha~'
